...
int main()
{
    ...
    // Take an input for noise
    cout<<"Enter The noise:\n";
    cin>>noisedBm;
    ...
    double noise_rms = std::sqrt(1e-3 * pow(10, noisedBm / 10)); // RMS amplitude of noise signal
  ...
  outfile.close();
    ...
    // Save original signal with noise to file
    ...
    //cout<<s<<endl << s.size(); checking the matrix, python truncates all the small values on its own, hence error! C++ op
